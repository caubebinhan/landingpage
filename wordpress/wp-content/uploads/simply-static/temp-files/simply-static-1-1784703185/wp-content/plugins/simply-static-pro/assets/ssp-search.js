...
try{window.dispatchEvent(new CustomEvent('ssp:index-ready'));}catch(_){}});let keys=['title','content','excerpt','language'];let fuse=null;loadConfig(function(response){config=JSON.parse(response);fuse=new Fuse(searchResults,{keys:keys,shouldSort:true,threshold:config.threshold?config.threshold:0.1,maxPatternLength:50});configLoaded=true;if(indexLoaded&&searchResults.length>0){fuse.setCollection(searchResults);}
