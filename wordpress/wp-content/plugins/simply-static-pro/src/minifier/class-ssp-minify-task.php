...
class Minify_Task extends Simply_Static\Task {
 ...
 public function perform() {
        ...
        $processed = $this->minify_files( $files );
        $total     = is_array( $files ) ? count( $files ) : 0;

        // Use a formatted message to avoid translation issues with unformatted placeholders.
        /* translators: 1: processed count, 2: total files */
        $this->save_status_message( sprintf( __( 'Minified %1$d of %2$d pages/files', 'simply-static-pro' ), (int) $processed, (int) $total ) );
	...
	public function get_excluded_files() {
		$css_urls = $this->options->get('minify_css_exclude');
		$js_urls  = $this->options->get('minify_js_exclude');
		$urls     = [];

		if ( $css_urls ) {
			$urls = array_unique( array_filter( Util::string_to_array( $css_urls ) ) );
		}

		if ( $js_urls ) {
			$js_urls = array_unique( array_filter( Util::string_to_array( $js_urls ) ) );
			if ( $js_urls ) {
				$urls = array_merge( $urls, $js_urls );
			}
		}
		...
		foreach ( $urls as $url ) {
			...
			$relative_url = Util::get_path_from_local_url( $url );
			$path         = implode( DIRECTORY_SEPARATOR, explode( '/', $relative_url ) );
			$full_path    = Util::add_trailing_directory_separator( $archive_dir ) . Util::remove_leading_directory_separator( $path );
			$paths[]      = $full_path;
		}
  ...
  *
  * @return int Number of files processed (minified)
  */
