...
class Form_Meta {
    ...
    public function add_meta_boxes( $post_type, $post ) {
        // Only run on the ssp-form custom post type.
        ...
        // Remove title.
        remove_post_type_support( 'ssp-form', 'title' );
    ...
    public function register_meta_fields() {
        // Deprecated: form_installation meta removed
        ...
        register_meta( 'post', 'form_custom_headers', array(
                'object_subtype' => 'ssp-form',
                'single'         => true,
                'show_in_rest'   => true,
        ) );
        ...
        // Optional: hidden input name for "Other Plugin" support
        register_meta( 'post', 'form_hidden_name', array(
                'object_subtype' => 'ssp-form',
                'single'         => true,
                'show_in_rest'   => true,
        ) );
        ...
        // Keep form_height registered for backward compatibility, but the setting is no longer used.
        register_meta( 'post', 'form_height', array(
                'object_subtype' => 'ssp-form',
                'single'         => true,
                'show_in_rest'   => true,
        ) );

        // New: unit for manual form height. Defaults to px for back-compat.
        register_meta( 'post', 'form_height_unit', array(
                'object_subtype' => 'ssp-form',
                'single'         => true,
                'show_in_rest'   => true,
                'default'        => 'px',
        ...
        register_meta( 'post', 'form_email_recipient', array(
                'object_subtype' => 'ssp-form',
                'single'         => true,
                'show_in_rest'   => true,
        ) );
    ...
    public function save_meta( object $request ) {
        if ( $request->get_params() ) {
            ...
            if ( ! $post_id ) {
                return [ "status" => 400, "message" => "Missing post_id" ];
            }
        ...
        return [ "status" => 400, "message" => "No parameters provided" ];
    }
    ...
    public function get_meta( object $request ) {
        if ( $request->get_params() ) {
            ...
            if ( ! empty( $meta ) ) {
                return [ "status" => 200, "message" => "Ok", "data" => $meta ];
            } else {
                return [ "status" => 400, "message" => "Empty value", "data" => '' ];
            }
        }
    }
