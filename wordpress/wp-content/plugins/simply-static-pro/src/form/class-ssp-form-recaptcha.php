...
/**
 * Google reCAPTCHA v3 integration for Simply Static Pro.
 *
 * Responsibilities:
 * - Enqueue the reCAPTCHA v3 browser script when enabled and a Site Key exists.
 ...
 *
 * The captcha widget markup is injected by Form_Patcher::inject_recaptcha_widget()
 * during static export. This class is server/runtime only.
 */
class Form_Recaptcha {
    /** @var object|null */
    ...
    public function __construct() {
        ...
        // Only initialize if reCAPTCHA v3 is selected
        ...
        $site_key     = isset( $options['recaptcha_site_key'] ) ? trim( (string) $options['recaptcha_site_key'] ) : '';
        $secret_key   = isset( $options['recaptcha_secret_key'] ) ? trim( (string) $options['recaptcha_secret_key'] ) : '';

        // Enqueue the reCAPTCHA loader when either forms or comments are enabled and a Site Key is present.
        ...
        if ( ! empty( $secret_key ) ) {
            add_action( 'rest_api_init', array( $this, 'register_recaptcha_rest' ) );
        }
    }

    /**
     * Enqueue Google reCAPTCHA v3 script (frontend).
     */
    public function enqueue_recaptcha_script() {
        ...
        $handle = 'ssp-google-recaptcha';
        if ( ! wp_script_is( $handle, 'enqueued' ) && ! wp_script_is( $handle, 'registered' ) ) {
            wp_enqueue_script( $handle, 'https://www.google.com/recaptcha/api.js?render=' . esc_attr( $site_key ), array(), null, true );
    ...
    /**
     * Verify Google reCAPTCHA v3 token for WordPress comments.
     *
    ...
    public function verify_comment_captcha( $commentdata ) {
        $options = get_option( 'simply-static' );
        $secret  = isset( $options['recaptcha_secret_key'] ) ? trim( (string) $options['recaptcha_secret_key'] ) : '';
        if ( empty( $secret ) ) {
            return $commentdata;
        }
    ...
    /**
     * Register REST routes for reCAPTCHA verification and optional proxy submit.
     */
    public function register_recaptcha_rest() {
        register_rest_route( 'simplystatic/v1', '/recaptcha/verify', array(
            'methods'             => 'POST',
            ...
            'callback'            => function ( \WP_REST_Request $request ) {
                ...
                $ok = $this->verify_recaptcha_token( $token, $ip );
                return new \WP_REST_Response( array( 'success' => (bool) $ok ), $ok ? 200 : 400 );
            },
        ) );

        register_rest_route( 'simplystatic/v1', '/recaptcha/submit', array(
            'methods'             => 'POST',
            'permission_callback' => '__return_true',
            'callback'            => function ( \WP_REST_Request $request ) {
                $params     = $request->get_params();
                $token      = (string) ( $params['g-recaptcha-response'] ?? '' );
                ...
                if ( ! $this->verify_recaptcha_token( $token, $ip ) ) {
                    if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                        error_log( '[SSP] reCAPTCHA submit: captcha_failed' );
                    }
                ...
                $allowed  = apply_filters( 'ssp_recaptcha_allowed_forward', $is_https, $forward_to, $params );
                if ( ! $forward_to || ! $allowed ) {
                    if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                        error_log( '[SSP] reCAPTCHA submit: forward_not_allowed url=' . $forward_to );
                    }
                ...
                if ( is_wp_error( $resp ) ) {
                    if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                        error_log( '[SSP] reCAPTCHA submit: forward_failed ' . $resp->get_error_message() );
                    }
                ...
                if ( ! $success && defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                    ...
                    error_log( '[SSP] reCAPTCHA submit: upstream_error status=' . $status . ' body_snippet=' . $snippet );
                }
    ...
    /**
     * Call Google siteverify for a given token using the stored Secret key.
     *
     * @param string      $token
     * @param string|null $remote_ip
     * @return bool
     */
    private function verify_recaptcha_token( $token, $remote_ip = null ) {
        $options = get_option( 'simply-static' );
        $secret  = isset( $options['recaptcha_secret_key'] ) ? trim( (string) $options['recaptcha_secret_key'] ) : '';
        ...
        // reCAPTCHA v3 returns a score (0.0 - 1.0). We consider scores >= 0.5 as valid.
        // You can adjust this threshold via filter if needed.
        $min_score = apply_filters( 'ssp_recaptcha_min_score', 0.5 );

        if ( empty( $json['success'] ) ) {
            return false;
        }

        // If score is present, check it meets minimum threshold
        if ( isset( $json['score'] ) && (float) $json['score'] < $min_score ) {
            return false;
        }
