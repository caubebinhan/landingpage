...
/**
 * Forminator compatibility for Form Entries (admin list formatting + meta enrichment).
 *
 * Ensures saved entries include the correct form title and plugin slug, and renders
 * a clean formatted payload by skipping Forminator internal fields.
 */
class Forminator {

    public function __construct() {
        // Populate saved entry meta based on Forminator payload
     ...
     * Skips internal Forminator/WP fields and prints scalar values.
     *
    ...
    /**
     * Populate FormEntry metadata from Forminator submission payload.
     *
    ...
    public function form_set_data( $entry ) {
        ...
        // Forminator typically posts numeric form_id.
        $form_id = 0;
        if ( isset( $decoded['form_id'] ) ) {
            $form_id = absint( $decoded['form_id'] );
        } elseif ( isset( $decoded['forminator_form_id'] ) ) {
            $form_id = absint( $decoded['forminator_form_id'] );
        }

        if ( $form_id > 0 ) {
            $entry->form_id     = $form_id;
            $entry->form_plugin = 'forminator';
            // Forminator forms are stored as posts (post type forminator_forms), so get_the_title works.
...
// Bootstrap
if ( class_exists( __NAMESPACE__ . '\\Forminator' ) ) {
    new Forminator();
}
