...
/**
 * Bricks Forms compatibility for Form Entries (admin list formatting + meta enrichment).
 */
class Bricks {

    public function __construct() {
        // Populate saved entry meta based on Bricks payload
     ...
     * Skips internal Bricks/WP fields and prints scalar values.
     *
    ...
    /**
     * Populate FormEntry metadata from Bricks submission payload.
     *
    ...
    public function form_set_data( $entry ) {
        ...
        // Try common identifiers for Bricks forms
        $form_id = 0;
        if ( isset( $decoded['bricks_form_id'] ) ) {
            $form_id = absint( $decoded['bricks_form_id'] );
...
// Bootstrap
if ( class_exists( __NAMESPACE__ . '\\Bricks' ) ) {
    new Bricks();
}
