...
class CORS {
    ...
    // Allow both new and legacy secret headers for compatibility
    header( 'Access-Control-Allow-Headers: Origin, X-Requested-With, X-WP-Nonce, X-Simply-Static-Secret, X-Simply-Static-Studio-Secret, Content-Type, Accept, Authorization ' );

    // If a shared secret is set, expose both header names so clients can pick either.
    $secret = '';
    ...
    if ( $secret ) {
        header( 'X-Simply-Static-Secret:' . $secret );
        header( 'X-Simply-Static-Studio-Secret:' . $secret );
    }
