...
class Basic_Auth {
	...
	public function __construct() {
		...
		add_action( 'update_option_simply-static', [ $this, 'update_htaccess' ], 20, 2 );
	}

	public function update_htaccess( $old_value, $new_value ) {
	...
	public function auth() {
		...
		$enabled = apply_filters( 'ssp_basic_auth_enabled', $options->get( 'http_basic_auth_on' ) );

		if ( ! $enabled ) {
			return;
		}

		// Check if URL contains an exclude string.
		$exclude_from_auth = [
			'ssp-form',
			'wp-json',
			'graphql',
		...
		// Also exclude iframe URLs.
		if ( ! empty( $options->get( 'iframe_urls' ) ) ) {
			$iframe_urls = explode( "\n", $options->get( 'iframe_urls' ) );

			// Add these to the excluded list.
			$exclude_from_auth = array_merge( $exclude_from_auth, $iframe_urls );
		};

		$exclude_from_auth = apply_filters( 'ssp_exclude_auth_list', $exclude_from_auth );
		$request_uri       = esc_html( $_SERVER['REQUEST_URI'] );

		foreach ( $exclude_from_auth as $term ) {
			if ( strpos( $request_uri, $term ) !== false ) {
				return;
			}
		}
