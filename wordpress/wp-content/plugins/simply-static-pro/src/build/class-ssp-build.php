...
class Build {
	...
	public function __construct() {
		...
		add_action( 'ss_before_perform_archive_action', array( $this, 'clear_build' ), 10, 2 );
	...
	public static function add_urls( int $build_id ) {
		...
		foreach ( $urls as $url ) {
			if ( Simply_Static\Util::is_local_url( $url ) ) {
				...
				$static_page->set_status_message( __( "Build URL", 'simply-static-pro' ) );
	...
	/**
	 * Convert Build Files/Directories to URLs and add them to the database.
	 *
	 * @param int $build_id current build id.
	 *
	 * @return void
		...
		foreach ( $additional_files as $item ) {
			if ( file_exists( $item ) ) {
				if ( is_file( $item ) ) {
					...
					$static_page->set_status_message( __( "Build File", 'simply-static-pro' ) );
					$static_page->build_id    = $build_id;
					$static_page->found_on_id = 0;
					$static_page->save();
				} else {
					...
					foreach ( $iterator as $file_name => $file_object ) {
						...
						$static_page = Simply_Static\Page::query()->find_or_initialize_by( 'url', $url );
						$static_page->set_status_message( __( "Build Dir", 'simply-static-pro' ) );
						$static_page->build_id    = $build_id;
						$static_page->found_on_id = 0;
						$static_page->save();
					}
	 ...
	 * @return void
	 */
	public function clear_build( $blog_id = 0, $action = '' ) {
		// Don't clear build ID on pause or resume - we need it to continue the build
	...
	/*
	 * Filter total pages by build ID for logging.
	 *
