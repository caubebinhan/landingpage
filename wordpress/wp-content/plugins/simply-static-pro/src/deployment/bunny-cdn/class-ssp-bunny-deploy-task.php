...
class Bunny_Deploy_Task extends Simply_Static\Task {
 ...
 public function perform() {
        ...
        Bunny_Updater::reset_counters();

        $done = $this->process_pages();

        if ( $done ) {
            Bunny_Updater::add_remaining_files();
            ...
            // Summary logging.
            $counters = Bunny_Updater::get_counters();
            \Simply_Static\Util::debug_log( sprintf( '[BunnyCDN] Deployment finished. Queued: %d | Uploaded: %d | Failed: %d | Skipped: %d', $counters['queued'], $counters['uploaded'], $counters['failed'], $counters['skipped'] ) );
        }
 ...
 protected function process_page( $static_page ) {
        ...
        if ( ! is_dir( $file_path ) && file_exists( $file_path ) ) {
            Bunny_Updater::add_file( $file_path, $this->batch_size );
        }
  ...
  if ( $this->options->get( 'generate_404' ) && realpath( $this->temp_dir . '404/index.html' ) ) {
            ...
            if ( $error_file_path ) {
                \Simply_Static\Util::debug_log( '[BunnyCDN] Uploading custom 404 error page: ' . $error_file_path );
