...
class Github_Database {
	...
	public function create_blob( string $file_path, string $relative_path, string $content ) {
		...
		if ( isset( $file_info['extension'] ) && in_array( $file_info['extension'], $utf8_mime_types, true ) ) {
			return array(
				'path'    => $relative_path,
				'mode'    => '100644',
				'type'    => 'blob',
			...
			try {
				...
				if ( ! isset( $response['url'] ) ) {
					Util::debug_log( 'We could not convert the file to a blob: ' . $file_path );
				}
	...
	public function create_tree( array $blobs ) {
		...
		try {
			// Get the base tree (current HEAD tree for the branch) to base our changes on.
			$base_tree = $this->client->get_tree( $this->user, $this->repository, $this->branch );

			if ( ! empty( $base_tree['sha'] ) ) {
		...
		// Prepare blobs: if entries carry a temporary 'file_path', load their content now.
		$prepared_blobs = [];
		foreach ( $blobs as $blob ) {
			$prepared = $this->prepare_blob_with_content( $blob );
			if ( null !== $prepared ) {
				$prepared_blobs[] = $prepared;
			}
		}

		if ( empty( $prepared_blobs ) ) {
			return false;
		}

		// Build the tree incrementally to avoid oversized payloads.
		$chunk_size       = apply_filters( 'ssp_github_tree_chunk_size', 100 );
		$current_tree_sha = $base_tree_sha;

		try {
			foreach ( array_chunk( $prepared_blobs, absint( $chunk_size ) ?: 100 ) as $chunk_index => $chunk ) {
				$tree_response    = $this->client->create_tree( $this->user, $this->repository, $current_tree_sha, $chunk );
				$current_tree_sha = isset( $tree_response['sha'] ) ? $tree_response['sha'] : $current_tree_sha;
			}
		} catch ( \Exception $e ) {
			Util::debug_log( 'GitHub create_tree failed (chunked). Error: ' . $e->getMessage() );

			return false;
		}

		return array(
			'tree-sha'      => $current_tree_sha,
	...
	public function commit( string $message, array $tree_data ) {
		...
		// Only include parents if we have a base tree SHA.
		if ( ! empty( $tree_data['base-tree-sha'] ) ) {
			$commit_data['parents'] = array( $tree_data['base-tree-sha'] );
		}
	...
	public function get_rate_limits(): array {
		...
		$limits = get_option( 'ssp_github_rate_limits', null );

		if ( null !== $limits ) {
			$limits['core'] = $limits['limit'];
