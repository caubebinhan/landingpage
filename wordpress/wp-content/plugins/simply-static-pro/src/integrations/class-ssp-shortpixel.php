...
class Shortpixel extends Integration {
 ...
 public function __construct() {
        $this->name = __( 'Shortpixel', 'simply-static-pro' );
        $this->description = __( 'Optimizes Images before exporting them for static sites.', 'simply-static-pro' );
    }
	...
	public function download_files( $file_urls ) {
		...
		// Return how many items we actually processed (saved or dequeued due to error)
	...
	public function save_to_optimized( $file_info ) {
		...
		$optimized[ $file_info['key'] ] = $file_info;
	...
	public function save_file( $file_info ) {
		...
		$lossy_url = isset($file_info['LossyURL']) ? $file_info['LossyURL'] : '';

		// Guard: if ShortPixel reports no lossy URL (NA), skip and dequeue.
		if ( ! $lossy_url || strtoupper( $lossy_url ) === 'NA' ) {
			Util::debug_log('ShortPixel: No LossyURL available for ' . ( $file_info['img_url'] ?? 'unknown img_url' ) . ' — removing from queue.');
			$this->remove_from_queue( $file_info );
			return;
		}
		...
		if ( is_wp_error( $file ) ) {
			Util::debug_log('ShortPixel: download_url error for LossyURL ' . $lossy_url . ' => ' . $file->get_error_message());
			$this->remove_from_queue( $file_info );
			return;
		}
		...
		if ( $file ) {
			...
			if ( ! $page ) {
				...
				Util::debug_log('ShortPixel: Page not found for URL ' . $img_url . ' — removing from queue.');
				$this->remove_from_queue( $file_info );
				@unlink($file);
				return;
			}
			...
			$dest_archive = $dir . $page->file_path;
			$orig_dir = dirname( $path );
			$archive_dir = dirname( $dest_archive );
			if ( ! is_dir( $orig_dir ) ) {
				wp_mkdir_p( $orig_dir );
			}
			...
			$copy = copy( $file, $path ); // Replace in uploads

			if ( $copy ) {
				// Also copy into the archive (static export working dir)
				$copy_archive = copy( $file, $dest_archive );
				Util::debug_log(sprintf('ShortPixel: Copied optimized image. Bytes=%s, uploads_dest=%s, archive_dest=%s, archive_copy=%s',
					$download_size ?: 'unknown', $path, $dest_archive, $copy_archive ? 'ok' : 'fail'));

				$this->save_file_webp( $file_info, $page );
				$this->remove_from_queue( $file_info );
				$this->save_to_optimized( $file_info );
			} else {
				Util::debug_log('ShortPixel: Failed to copy optimized image to uploads path ' . $path);
			}
	...
	public function save_file_webp( $file_info, $page = null ) {
		if ( ! $this->is_webp_enabled() ) {
			return;
		}

		$lossy_url = $file_info['WebPLossyURL'];

		if ( ! $lossy_url || 'NA' === strtoupper( $lossy_url )) {
			return;
		}
		...
		if ( $file ) {
			...
			if ( $page ) {
				$file_info = pathinfo( $page->file_path );
				$webp_path = $file_info['dirname'] . '/' . $file_info['filename'] . '.webp';
				// Static exported file.
				copy( $file, $dir . $webp_path );
			}
	...
	public function is_webp_enabled() {
		if ( $this->get_options()->get( 'shortpixel_webp_enabled' ) ) {
			return true;
		}
	 ...
	 * @param bool   $fix_url If true and there is no Post ID found, try to fix the URL.
	 *
	 * @return int
	 */
	public static function get_attachment_id_from_image( $url, $fix_url = true ) {
		...
		if ( ! $post_id && $fix_url ) {
			...
			$post_id = self::get_attachment_id_from_image( $url, false );
		}
	...
	/**
	 * This method for now tries to fix the URL where the domain + wp-content is concatenated without '/'-
	 * Example: https://example.comwp-content/uploads/2023/01/image.jpg (notice .com and wp-content).
	 *
	...
	public static function fix_url( $url ) {
		...
		// Seems the domain + '/' is there, nothing to fix for now.
		if ( str_starts_with( $url,  trailingslashit( $site_url ) ) ) {
			return $url;
		}

		$url = str_replace( $site_url, trailingslashit( $site_url ), $url );
